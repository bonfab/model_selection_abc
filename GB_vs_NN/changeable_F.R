...
F_layer <- function(K, F_values, number_locus = sample(40000 - 8000, 1) + 8000){
  ...
  ancestral_pop <- rep(0.5, number_locus)
...
PCA_summary <- function(data, reduce_to = 16){
...
generate <- function(K, number_locus = sample(20000 - 2000, 1) + 2000, number_admixed = floor(rbeta(1, 1, 1.2) * K), pop_sizes = rdirichlet(1, rep(1, K + number_admixed)), sample_size = sample(1200 - 200, 1) + 200, F_value = 0.001){
  
  F_values <- rep(F_value, K)
...
make_data <- function(samples = 250, populations = 2:12){
  ...
  for(p in labels){
    ...
    pop <- t(sapply(p, function(y) PCA_summary(generate(y, number_locus = sample(30000- 3000, 1)+3000))))
  ...
  saveRDS(list(result, labels), "./data_K/lower_F_1.rds")
