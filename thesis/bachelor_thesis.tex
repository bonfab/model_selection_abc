...
Gradient boosting is a supervised learning method for classification and regression, that iteratively adds basis learners to a linear combination to reduce an arbitrary loss function.\\
...
Gradient boosting, firstly developed and described by \cite{freund1997decision, friedman2001greedy, friedman2002stochastic}, circumvents the necessity of differentiability by growing the ensemble of base learners iteratively with respect to minimising the empirical risk. The method it employs is also called forward stagewise additive modeling.\\

The algorithm works as following:\\
Let $H \subset \mathcal{H}$ be the chosen set of possible base learners.
\begin{enumerate}
\item $f_0(x) = 0$
\item For each stage $i \in 1,\ldots,M$:
\begin{enumerate}
\item add a base learner $h_i(x)$ that reduces the residual  
\end{enumerate}
\end{enumerate}
