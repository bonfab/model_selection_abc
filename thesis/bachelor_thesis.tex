...
As an alternative, a greedy approximation approach can be used. The greedy algorithm chooses the best dimension $X_j$ and splitting point $s$ that minimises the loss for the two new regions that arise (w.l.o.g. only binary trees will be considered). The two new regions are defined as:
\begin{align*}
R_1(j, s) = \{X | X_j \leq s\} \qquad R_2(j, s) = \{X | X_j > s \}
\end{align*}

The feature space is divided by a plane that is orthogonal to the axis corresponding to feature $X_j$. The location of the plane is positioned at the value of $s$ on that axis. \\
